...
![Top Elements versus non-Elements contrasts](figures/bar_elements_vs_non_elements_top_contrasts.png)
...
![Subject zones by intellectual-value markers](figures/heatmap_subject_intellectual_rates.png)
...
![Bridge routes across Elements and non-Elements mathematical print](figures/heatmap_bridge_route_marker_rates.png)
...
![Elements postures by selected markers](figures/heatmap_elements_mode_marker_rates.png)
...
![Elements book groups by selected markers](figures/heatmap_elements_book_group_marker_rates.png)
...
![Figure and diagram functions by format](figures/heatmap_figures_diagrams_by_format.png)

![Elements title-page modes by format](figures/heatmap_format_elements_modes.png)

![Elements social and intellectual markers by format](figures/heatmap_format_elements_markers.png)
...
### Jesuit Visibility Inside And Outside The Elements

The Jesuit signal becomes clearer when Jesuit-marked *Elements* representatives are compared with Jesuit-marked books elsewhere in the mathematical corpus through 1705. Here “Jesuit-marked” means that the title-page classification makes the affiliation visible; it is not a complete census of Jesuit authorship, ownership, or teaching.

Among 24 Jesuit-marked *Elements* representatives, 17 (70.8%) advertise method or order and 12 (50.0%) advertise correction or revision. Among 17 Jesuit-marked non-Elements representatives, direct practical utility is more prominent (7 cases, 41.2%), and the books range through perspective, instruments, mechanics, astronomy/navigation, music, and mixed mathematical compendia. Ancient authority remains nearly identical between Jesuit-marked and other *Elements* editions, so Jesuit distinction does not consist simply in making Euclid more ancient or more canonical.

The historical distinction is instead functional. Inside the *Elements*, visible Jesuit identity attaches especially to corrected, methodized, institutionally portable Euclid. Outside it, Jesuit visibility attaches more strongly to applied mathematical arts. Jesuit print culture therefore does not form one subject category; it supplies different institutional forms for different mathematical objects. Supporting counts are in [`tables/jesuit_corpus_marker_rates_pre_1706.csv`](tables/jesuit_corpus_marker_rates_pre_1706.csv).
...
### Controlled Same-Person Comparisons

The mediation pattern survives a stricter comparison. Within represented same-person, same-city, same-language windows of no more than five years, *Elements* title pages often add ancient authority, correction, translation, augmentation, selection, or method, while neighboring non-Elements works by the same person more often advertise direct utility or procedure. This controls several obvious sources of title-page variation without pretending that the corpus contains complete bibliographies for every author or editor.

Denis Henrion is the strongest repeated anchor: his controlled *Elements* cases show substantially more correction, translation, augmentation, and ancient authority than his neighboring works in this corpus. Jean Errard and Jacques Ozanam show that professional and practical social worlds can continue across the corpus boundary while the *Elements* adds translation, commentary, selection, revision, or correction. The comparison also prevents overstatement. Pierre Forcadel's *Elements* title pages do not consistently carry stronger method rhetoric, and Marin Mersenne produces only a weak contrast. The result is therefore a recurring relationship, not a universal formula: entering the *Elements* often changes the advertised work performed on a canonical text.

The controlled summary and complete pair evidence are retained in [`tables/controlled_same_person_summary.csv`](tables/controlled_same_person_summary.csv) and [`tables/controlled_same_person_pairs.csv`](tables/controlled_same_person_pairs.csv).
...
![Deductive and mathematical parts by corpus](figures/heatmap_deductive_parts_by_corpus.png)

![Figure and diagram functions by corpus](figures/heatmap_figures_diagrams_by_corpus.png)
...
### Print Geography And Mobility

Mobility is visible not only in title-page rhetoric but also in the corpus's observed print geography. This evidence records print locations, reprint links, and explicit language transfer; it does not reconstruct where books were sold, owned, read, or taught, so “circulation” would be too strong a label.

The pre-1705 *Elements* corpus is locally anchored but not simply local. Among 65 reprint clusters containing at least two Elements editions, 46 (70.8%) remain in one city and 63 (96.9%) remain in one language. Reprinting therefore usually deepens a local and linguistic print center. A smaller group follows identifiable cross-city routes, including Lyon-Geneva, Paris-Cologne, Urbino-Pesaro, Ingolstadt-Bologna, Rotterdam-Utrecht, and Cologne-Paris. Explicit language-transfer metadata is rarer and clusters especially around Greek- or Latin-to-French translation, followed later by French-to-English transfer.

The result sharpens the report's language of mediation. The *Elements* did not diffuse through one smooth pan-European network. Its observed print reach combines local serial depth, regional vernacular consolidation, stable Latin institutional routes, and a smaller number of multilingual adaptations. The supporting reprint and translation evidence is in [`tables/elements_pre_1706_reprint_cluster_summary.csv`](tables/elements_pre_1706_reprint_cluster_summary.csv) and [`tables/elements_pre_1706_language_transfer_cases.csv`](tables/elements_pre_1706_language_transfer_cases.csv).
...
The appendices are in [`appendices.md`](appendices.md).
