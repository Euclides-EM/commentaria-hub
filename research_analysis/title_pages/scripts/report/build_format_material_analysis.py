...
ITEMS_PRINT = ROOT.parents[2] / "ocrflow" / "store" / "items_metadata" / "items_print.csv"
...
def main() -> None:
    ...
    book_dist.add_prefix("count_").join(book_pct.add_prefix("pct_within_format_")).reset_index().to_csv(
        TABLES / "format_elements_book_group_distribution.csv", index=False
    )

    mode_matrix = rate_matrix(elements, "format_group", MODE_MARKERS)
    mode_matrix.to_csv(TABLES / "format_elements_mode_rates_matrix.csv")
    save_heatmap(
    ...
    marker_matrix.to_csv(TABLES / "format_elements_marker_rates_matrix.csv")
    save_heatmap(
    ...
    pd.DataFrame(density_rows).set_index("format_group").pipe(ordered).reset_index().to_csv(
        TABLES / "format_elements_density_summary.csv", index=False
    )
    ...
    pd.DataFrame(subject_rows).set_index("format_group").pipe(ordered).reset_index().to_csv(
        TABLES / "format_full_corpus_subject_rates.csv", index=False
    )
