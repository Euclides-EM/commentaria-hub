...
META = ROOT.parents[2] / "ocrflow" / "store" / "items_metadata"
OUT = ROOT / "results" / "report" / "tables"
BASE_KEYS = ROOT / "data" / "base_keys" / "title_page_elements_print_geography_keys.txt"
...
def build_translation_edges(df: pd.DataFrame, title_page: pd.DataFrame, scope_name: str) -> None:
    ...
    out.to_csv(OUT / f"{scope_name}_language_transfer_cases.csv", index=False)
    if len(out):
        edges = (
        ...
        edges.to_csv(OUT / f"{scope_name}_language_transfer_edges.csv", index=False)
...
def main() -> None:
    ...
    frozen_keys = {line.strip() for line in BASE_KEYS.read_text(encoding="utf-8").splitlines() if line.strip()}
    live_keys = set(elements["key"].dropna().astype(str))
    missing = sorted(frozen_keys - live_keys)
    if missing:
        raise ValueError(f"Frozen print-geography keys missing from metadata: {missing}")
    ignored_new = sorted(live_keys - frozen_keys)
    if ignored_new:
        print(f"Ignoring {len(ignored_new)} live metadata keys outside the frozen study corpus: {ignored_new}")
    elements = elements[elements["key"].astype(str).isin(frozen_keys)].copy()
    ...
    summaries = []
    for scope_name in ["elements_all", "elements_pre_1706"]:
