...
func ApplyStretchTowardsCategory(c *Root, stc *StretchTowardsCategory) error {
	...
	if !ok {
		log.Printf("WARN: stretch category %q not found in categories, skipping", stc.StretchCategory)
		return nil
	}

	tgtCatID, ok := catByName[stc.Towards]
	if !ok {
		log.Printf("WARN: towards category %q not found in categories, skipping", stc.Towards)
		return nil
	}
	...
	for imgID, entry := range byImage {
		...
		for _, src := range entry.src {
			var strechingSides []Side
			switch stc.ContactSide {
			case SideAll:
				strechingSides = []Side{SideLeft, SideRight, SideTop, SideBottom}
			case SideHorizontal:
				strechingSides = []Side{SideLeft, SideRight}
			case SideVertical:
				strechingSides = []Side{SideTop, SideBottom}
			default:
				strechingSides = []Side{stc.ContactSide}
			}

			for _, side := range strechingSides {
				tgt := chooseTarget(src, entry, side, stc.ContactType)
				if tgt == nil {
				...
				case ContactTypeInner:
					...
					case SideBottom:
						...
						newH = bottom - sy
					default:
						return fmt.Errorf("unsupported contact side %q for image %d", side, imgID)
					}
...
func chooseTarget(src *Annotation, entry *imgAnns, side Side, contactType ContactType) *Annotation {
	...
	for _, tgt := range entry.tgt {
		...
		case ContactTypeInner:
			...
			case SideTop:
				if intervalsIntersect(sLeft, sRight, tLeft, tRight) &&
					...
					match = true
					// how far up we can stretch inside
					dist = sTop - tTop
					if dist < 0 {
						match = false
					}
			...
			case SideBottom:
				if intervalsIntersect(sLeft, sRight, tLeft, tRight) &&
					...
					match = true
					// how far down we can stretch inside
			...
			case SideLeft:
				if intervalsIntersect(sTop, sBottom, tTop, tBottom) &&
					...
					match = true
					// how far left we can stretch inside
					dist = sLeft - tLeft
					if dist < 0 {
						match = false
					}
			...
			case SideRight:
				if intervalsIntersect(sTop, sBottom, tTop, tBottom) &&
					...
					match = true
					// how far right we can stretch inside
					dist = tRight - sRight
					if dist < 0 {
						match = false
					}
		...
		case ContactTypeOuter:
			...
			case SideTop:
				if intervalsIntersect(sLeft, sRight, tLeft, tRight) &&
					tBottom <= sTop {
					match = true
					// gap from src top to target bottom
					dist = sTop - tBottom
					if dist < 0 {
						match = false
					}
		...
		// selection rule:
		// - inner: maximize stretch (pick largest dist)
		// - outer: minimize stretch (pick smallest dist)
		if !hasBest {
			hasBest = true
			bestDist = dist
			best = tgt
