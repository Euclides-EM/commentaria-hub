...
func (t *TEI) GetTEI(datasetId, key string, featureParams []string) ([]byte, error) {
	...
	if datasetId != "tps" {
		return nil, fmt.Errorf("TEI generation only supported for dataset 'tps'")
	}
...
func (t *TEI) buildAnnotatedText(text string, results []*feature.Result) (string, map[string]anchorInfo) {
	...
	for _, result := range results {
		for _, value := range result.Values {
			// Extract text from value (could be Root or from Children)
			valueText := value.Root
			if valueText == "" && len(value.Children) > 0 {
				// Try to get text from first child
				valueText = value.Children[0].Root
			}
			if valueText == "" {
				continue
			}
...
func (t *TEI) generateSpanFromAnchor(spanID int, anchorID string, info anchorInfo, respStmts []respStmtInfo) string {
	...
	if normalized == "" && len(info.value.Children) > 0 {
		normalized = info.value.Children[0].Root
	}
	if normalized == "" {
