...
func (e *Edition) ListEditions(expand []ocrflow.EditionExpandOptions, orderBy []ocrflow.EditionOrderByOptions) ([]*ocrflow.Edition, error) {
	eds, err := e.editionStore.ListEditions()
	if err != nil {
		return nil, fmt.Errorf("failed to list editions from store: %w", err)
	}
	for _, edition := range eds {
		if slices.Contains(expand, ocrflow.EditionExpandFacsimiles) {
...
func (e *Edition) CreateEdition(ed *ocrflow.Edition) error {
	ed.ID = ed.Name // for now, set name to ID
	return e.editionStore.InsertEdition(ed)
}

func (e *Edition) GetFacsimile(editionKey, facsimileID string) (*ocrflow.Edition, *ocrflow.Facsimile, error) {
...
func (e *Edition) CreateFacsimile(editionId string, f *ocrflow.Facsimile) (*ocrflow.Facsimile, error) {
