...
func (a *Annotation) ListAnnotations(id string) ([]*ocrflow.Annotation, error) {
...
func (a *Annotation) Get(datasetId, id string) (*ocrflow.Annotation, error) {
...
func (a *Annotation) Create(datasetID string, ann *ocrflow.Annotation) (*ocrflow.Annotation, error) {
	...
	if ann.Pages == "" {
		...
		pages, err := store.InferPages(imgPath, ocrflow.AnnotationFormatAlto)
		if err != nil {
...
func (a *Annotation) CreateFromZip(aum *ocrflow.AnnotationUploadMetadata, save func(dstPath string) error) (*ocrflow.Annotation, error) {
	...
	if aum.Format == ocrflow.AnnotationFormatYolo {
	...
	pages, err := store.InferPages(a.fileSysMgt.DatasetAnnotationAltoDir(ann), ocrflow.AnnotationFormatAlto)
	if err != nil {
...
func (a *Annotation) ApplyRules(datasetID string, id string, aar *annotationrule.ApplyRules) (*ocrflow.Annotation, error) {
...
func (a *Annotation) GetAnnotationIndex(datasetID, id string, categories []string) (*ocrflow.AnnotationIndex, error) {
	ann, err := a.Get(datasetID, id)
	if err != nil {
		return nil, fmt.Errorf("failed to get annotation: %w", err)
	}
	...
	return &ocrflow.AnnotationIndex{
		DatasetID:    datasetID,
		AnnotationID: id,
...
func (a *Annotation) Update(datasetID string, annotationID string, ann *ocrflow.Annotation) (*ocrflow.Annotation, error) {
...
func (a *Annotation) Duplicate(datasetID string, annotationID string, name string, description string) (*ocrflow.Annotation, error) {
...
func (a *Annotation) GetReviewByIndex(datasetID string, annotationID string, toReview *ocrflow.AnnotationExpectedBlocks) (*ocrflow.AnnotationExpectedBlocks, error) {
	ann, err := a.Get(datasetID, annotationID)
	if err != nil {
		return nil, fmt.Errorf("failed to get annotation: %w", err)
	}
	...
	var diffs []*ocrflow.SuggestedDiff
	for _, page := range pages {
	...
	if slices.Contains(toReview.SanityChecks, ocrflow.AnnotationExpectedBlocksSanityTypeExact) {
		if len(diffs) != len(toReview.ExpectedBlocks) {
			toReview.FailedChecks = append(toReview.FailedChecks, ocrflow.AnnotationExpectedBlocksSanityTypeExact)
		}
	}
...
func (a *Annotation) ListAnnotationIDsByUsedModels() (map[string][]*ocrflow.AnnotationReference, error) {
	anns1, err := a.annotationStore.ListAppliedRulesByAnnotationIDs()
	if err != nil {
		return nil, fmt.Errorf("failed to list annotations from store: %w", err)
	}
	modelToAnns := make(map[string][]*ocrflow.AnnotationReference)
	for datasetID, anns := range anns1 {
		for annID, appliedRules := range anns {
			...
			for _, modelID := range modelIDs {
				modelToAnns[modelID] = append(modelToAnns[modelID], &ocrflow.AnnotationReference{
					DatasetID: datasetID,
					ID:        annID,
				})
			}
...
func buildNodes(remainingCats []string, data []categoryPageContent) []*ocrflow.AnnotationIndexNode {
	...
	var nodes []*ocrflow.AnnotationIndexNode

	var wipNode *ocrflow.AnnotationIndexNode
	...
	for i, item := range data {
		if item.category == currentCat {
			...
			wipNode = &ocrflow.AnnotationIndexNode{
				Category: item.category,
				Content:  item.content,
				Location: ocrflow.AnnotationLocation{Page: item.page},
			}
