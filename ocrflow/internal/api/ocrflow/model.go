	...
	if expand == "used_in_annotations" {
		...
		for _, m := range models {
			if annIDs, ok := usedInAnnotations[m.ID]; ok {
				...
				m.UsedInAnnotations = []*ocrflow.AnnotationReference{}
			}
		}
	}
	return models, nil
...
func (h *Handlers) UploadModel(r *http.Request) (any, error) {
	...
	var baseAnnotations []*ocrflow.AnnotationReference
	for _, rawAnnID := range strings.Split(baseAnnotationsRaw, ",") {
		...
		baseAnnotations = append(baseAnnotations, &ocrflow.AnnotationReference{
			DatasetID: ids[0],
			ID:        ids[1],
		})
	}
...
func (h *Handlers) UpdateModel(r *http.Request) (any, error) {
	id := r.PathValue("id")
	var toUpdate ocrflow.Model
	if err := common.DecodeBody(r, &toUpdate); err != nil {
		return nil, err
	}
