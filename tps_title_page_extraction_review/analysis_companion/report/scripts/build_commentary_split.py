...
def main() -> None:
    ...
    corpus_matrix.to_csv(TABLES / "report_commentary_split_by_corpus_matrix.csv")
    save_heatmap(
        corpus_matrix,
    ...
    for group_col, min_den in [
        ...
        long.to_csv(TABLES / f"report_commentary_split_by_{group_col}_long.csv", index=False)
        matrix = pivot_rates(long)
        matrix.to_csv(TABLES / f"report_commentary_split_by_{group_col}_matrix.csv")
        if group_col in {"elements_books_group", "natural_dominant_mode", "period", "language_first"}:
            save_heatmap(
                matrix,
                FIGURES / f"heatmap_commentary_split_by_{group_col}.png",
