...
CASE_MATRIX = DERIVED / "author_editor_portfolio_case_matrix.csv"
PAIR_MATRIX = DERIVED / "author_editor_portfolio_elements_non_elements_pairs.csv"
...
def main() -> None:
    ...
    write_csv(DERIVED / "controlled_author_editor_close_pairs.csv", controlled, fields)
    write_csv(DERIVED / "controlled_author_editor_close_pair_shortlist.csv", shortlist, fields)
    write_csv(DERIVED / "controlled_author_editor_close_pair_summary.csv", summary_rows, list(summary_rows[0].keys()) if summary_rows else [])
