...
def main() -> None:
    ...
    compare_bool(df, "comparison_group", bool_cols).to_csv(
        OUT / "to_1705_jesuit_elements_non_elements_bool_comparison.csv", index=False
    )
    ...
    bools = pd.read_csv(OUT / "to_1705_jesuit_elements_non_elements_bool_comparison.csv")
    markers = [
        "practical / utility",
        "access / pedagogy",
        "method / order",
